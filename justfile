...
deploy-local hostname wipe="no" prompt="no" : _require_root
  ...
  just _extract_host_key "{{hostname}}" "{{prompt}}" "$temp_key"
...
deploy-remote hostname host_ip_addr wipe="no" reboot="yes":
  ...
  temp_key=$(mktemp)
  trap 'rm -f "$temp_key"' EXIT
  just _extract_host_key "{{hostname}}" "no" "$temp_key"
  ...
  ssh "${ssh_opts[@]}" "nixos@{{host_ip_addr}}" 'sudo bash -s' << EOF
    set -euo pipefail
    cd /tmp/nix_hosts
    {{nix_cmd}} shell nixpkgs#just --command just _run_build_sequence \
      "{{hostname}}" "{{wipe}}" "/tmp/secrets/host_keypair.age"
    rm -rf /tmp/secrets
EOF
...
[private]
_extract_host_key hostname prompt out_file:
  #!/usr/bin/env bash
  if [ "{{prompt}}" = "yes" ]; then
...
_execute_disko_format hostname:
  ...
  # Ensure the potentially exposed passphrase file is destroyed if Disko fails
  trap 'rm -f /tmp/zfs_passphrase' EXIT

  {{nix_cmd}} run "github:nix-community/disko" -- --mode format --flake ".#{{hostname}}"
  ...
  {{nix_cmd}} run "github:nix-community/disko" -- --mode mount --flake ".#{{hostname}}"
